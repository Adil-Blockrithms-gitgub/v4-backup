...
// import { useBuyMultipliers } from '@/hooks/useEclipseNFT';
...
const BuyMultiplier: FC<Props> = ({ handleClose, open, tokenId }) => {
  // const { mutationAsync, isPending } = useBuyMultipliers();
  ...
  const formik = useFormik({
    initialValues: {
      numberOfMultiplier: 1, // Initial value for the input field
    },
    validationSchema,
    enableReinitialize: true,
    onSubmit: async (values) => {
      try {
        await mutationAsync([Number(tokenId), values.numberOfMultiplier], parseEther("10"));
        ...
        <form onSubmit={formik.handleSubmit}>
          ...
          <Box mt={2}>
            <Button
              ...
              fullWidth
              color="primary"
              disabled={formik.values.numberOfMultiplier < 1 || isPending}
            >
              Submit
            </Button>
          </Box>
